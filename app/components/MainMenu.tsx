...
export default function MainMenu({
  ...
  const menuItems: MenuItem[] = [
    { label: canContinue ? "이어하기" : "시작하기", icon: "▶", action: onStart },
    ...
    { label: "종료", icon: "✕", action: () => { window.close(); setExitHint(true); } },
  ];
    ...
    <>
      <div className="relative h-[100dvh] w-full overflow-hidden bg-game-bg">
        ...
        <div
          ...
          style={{ background: "linear-gradient(to bottom, rgba(4,6,4,0.72) 0%, rgba(4,6,4,0.55) 40%, rgba(4,6,4,0.30) 65%, rgba(4,6,4,0.70) 100%)" }}
        />

        <div className="relative z-10 flex h-full flex-col items-center justify-center">
          <p className="text-[11px] md:text-[12px] tracking-widest mb-5 px-4 text-center font-mono text-[#7a9a40]">
            · 5·18민주화운동 인터랙티브 스토리 ·
          </p>
          ...
          <div className="relative mb-3 mx-4 md:mx-0">
            <div
              ...
              style={{
                ...
                boxShadow: "0 0 0 2px var(--color-game-border), 0 0 0 5px var(--color-game-panel), inset 0 0 30px rgba(74,106,26,0.12)",
              }}
            >
              <h1
                className="text-3xl md:text-5xl font-bold text-center text-game-text font-pixel tracking-[0.06em]"
                style={{ textShadow: "0 0 24px rgba(196,212,122,0.5), 3px 3px 0 #1a2a0c" }}
              >
                그날의 선택
              </h1>
            </div>
            {(["top-0 left-0 border-t-4 border-l-4", "top-0 right-0 border-t-4 border-r-4", "bottom-0 left-0 border-b-4 border-l-4", "bottom-0 right-0 border-b-4 border-r-4"] as const).map((cls) => (
              <div key={cls} className={`absolute w-5 h-5 border-game-text ${cls}`} style={{ margin: -7 }} />
            ))}
          </div>

          <p className="hidden md:block text-[13px] mb-12 tracking-widest px-4 text-center font-mono text-[#5a7a20]">
          ...
          <div className="flex flex-col items-center gap-2.5 w-full max-w-sm px-4 md:w-96 md:max-w-none md:px-0">
            {menuItems.map(({ label, icon, action }) => (
              <button
                ...
                className="w-full flex items-center gap-4 px-6 py-4 border transition-all cursor-pointer font-pixel"
                style={{
                  background: hovered === label ? "rgba(22,32,16,0.95)" : "rgba(6,9,4,0.88)",
                  borderColor: hovered === label ? "#6a9a2a" : "var(--color-game-border)",
                  boxShadow: hovered === label ? "0 0 0 1px var(--color-game-border-bright)" : "none",
                }}
              >
                <span
                  className="text-[14px] w-5"
                  style={{ color: hovered === label ? "var(--color-game-text)" : "var(--color-game-border-bright)" }}
                >
                  {icon}
                </span>
                <span
                  className="text-[14px]"
                  style={{ color: hovered === label ? "var(--color-game-text)" : "var(--color-game-accent)" }}
                >
                  {label}
                </span>
                {hovered === label && <span className="ml-auto text-[12px] text-game-border-bright">▶</span>}
              </button>
            ))}
          </div>

          <div className="absolute bottom-7 flex flex-col items-center gap-1.5">
            <p className="text-[11px] font-mono text-[#4a6a20]">역사를 기억하는 일은 오늘을 지키는 일입니다</p>
            <p className="text-[10px] font-mono text-[#344e16]">— 5.18 민주화운동 —</p>
            <p className="text-[10px] mt-1 font-mono text-[#263810]">© Remember 5.18</p>
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/[0.88]">
          ...
          >
            ...
            <button
              ...
              className="border border-game-border bg-game-panel hover:bg-[#162010] px-6 py-2.5 text-[12px] text-game-accent font-mono"
            >
              돌아가기
            </button>
          </div>
