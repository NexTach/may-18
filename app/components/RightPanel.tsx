...
function ChoiceBtn({
  ...
  return (
    <button
      ...
      className="w-full flex items-center gap-3 text-left transition-all min-h-[44px]"
      style={{
        background: hov ? "rgba(245,208,108,0.08)" : "var(--color-game-panel-dark)",
        border: "1.5px solid",
        borderColor: choice.disabled
          ? "var(--color-game-border)"
          : hov ? "var(--color-game-accent)" : "var(--color-game-border-bright)",
        padding: "11px 14px",
    ...
    >
      <span
        className="shrink-0 w-[26px] h-[26px] flex items-center justify-center text-[13px]"
        style={{
      ...
      <span className="flex-1">
        <span className="block text-[13px] xl:text-[14px] leading-relaxed font-mono"
          style={{ color: choice.disabled ? "var(--color-game-border-bright)" : "var(--color-game-text)" }}>
          {choice.text}
        </span>
        <span className="block mt-0.5 text-[11px] xl:text-[12px] leading-relaxed font-mono"
          style={{ color: "var(--color-game-text-dim)" }}>
          {choice.detail}
        </span>
        {choice.disabledReason && (
          <span className="block mt-0.5 text-[11px] xl:text-[12px] font-mono"
      ...
      {choice.stat && choice.statDelta !== undefined && (
        <span className="shrink-0 text-[12px] xl:text-[13px] font-bold font-mono"
...
export default function RightPanel({
  ...
  const npcRevealCount =
    revealIndex < 0
      ? 0
      : dialogue.slice(0, revealIndex + 1).filter((l) => l.avatar !== "player").length;
  ...
  return (
    <div className="flex flex-col bg-game-panel border-2 border-game-border overflow-hidden flex-1 md:flex-none md:w-85 md:min-w-85 xl:w-[380px] xl:min-w-[380px] 2xl:w-[440px] 2xl:min-w-[440px]"
      ...
      <button
        type="button"
        className="p-4 xl:p-5 border-b border-game-border cursor-pointer text-left flex flex-col shrink-0 md:flex-none min-h-[110px] xl:min-h-[130px]"
          ...
          {situationOpen && (
            <div className="flex-1 overflow-y-auto min-h-0 p-3 flex flex-col gap-3 bg-game-panel">
              <div className="border border-game-border bg-game-panel-dark px-3 py-2.5">
                <p className="text-[12px] text-game-text-dim leading-relaxed font-mono">{situation}</p>
              </div>
      ...
      {showChoices && (
        ...
        >
          <div className="mt-auto border-t border-game-border">
            <button
              type="button"
              onClick={() => { if (soundOn) playSfx("click"); setSheetOpen(true); }}
      ...
      {done && (
        ...
        >
          <div className="border border-game-border bg-game-panel px-3 py-2.5 xl:px-4 xl:py-3 shrink-0">
            ...
            <p className="text-[12px] xl:text-[13px] text-game-text-dim leading-relaxed font-mono">{situation}</p>
          </div>
          {visibleDialogue.map((l, i) => (
            <DialLine key={`${l.avatar}-${l.name}-${i}`} line={l} />
          ))}
