    ...
    >
      ...
      >
        ...
        <div className="border border-game-border bg-game-panel-dark p-4 xl:p-5">
          ...
          {choiceLog.length === 0 ? (
            <p className="text-[13px] xl:text-[14px] text-[#2a3a10] font-mono">
              아직 기록된 선택이 없습니다.
            </p>
          ) : (
            <div className="flex flex-col gap-1.5">
              {choiceLog.map((c, i) => (
                <div key={`${i}-${c}`} className="flex items-start gap-3">
                  ...
                  <span className="text-[13px] xl:text-[14px] text-[#7a9038] font-mono">
                    {c}
                  </span>
                </div>
              ))}
        ...
        <div className="flex items-center gap-5 mt-3 flex-wrap">
          {LEGEND_ITEMS.map(({ color, label }) => (
            <div key={label} className="flex items-center gap-1.5">
              <span
                className="w-3 h-3 xl:w-3.5 xl:h-3.5 inline-block border"
          ...
          {mode === "activity" && (
            <div className="flex items-center gap-1.5">
              <span className="text-[12px] xl:text-[13px] text-game-border-bright font-mono">
                - - -
              </span>
              <span className="text-[12px] xl:text-[13px] text-game-text-muted font-mono">
                경로
          ...
          {mode === "city" && (
            <div className="flex items-center gap-1.5">
              <span className="text-[11px] xl:text-[12px] text-game-border-bright font-mono">
                □ - -
              </span>
              <span className="text-[12px] xl:text-[13px] text-game-text-muted font-mono">
                게임 무대
